...
class Classement extends Component {
  pushToClassement = (id) => {
    ...
    if (use === -1) {
      ...
      this.props.dispatch({ type: 'client/addNotife', message: 'You have already voted for him.' })
    }
  }
          ...
          {this.props.classement.songs.map((s, key) => {
            return (
              <View key={key} style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                ...
                <Button style={{ backgroundColor: `rgb(${Math.floor(Math.random() * 255)},${Math.floor(Math.random() * 255)},${Math.floor(Math.random() * 255)})` }} kind='squared' iconName='md-musical-notes'>{s.name}</Button>
              </View>
            )
          })}
        </ScrollView>
