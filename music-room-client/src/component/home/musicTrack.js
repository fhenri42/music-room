...
componentDidMount(props){
  const { playlist, dispatch, user, playlistId } = this.props
  this.playTrackWrapper(0);
  const thisPlaylist = playlist.playlists.findIndex(e => e._id === playlistId)
  const songs = thisPlaylist.playlists[index1].songs
...
playTrackWrapper = (id) => {
  ...
  updateGrade = (grade, songId) => {
    ...
    const songs = thisPlaylist.playlists[index1].songs
    let newsongs = []
    songs[songId].vote += grade
    let i = 0
    while (i < songs.length){
      let y = 0
      while (y < songs.length){
        if (songs[i].grade > songs[y].grade && songs[i].vote < songs[y].vote){
          let gradeSong = songs[i].grade
          songs[i].grade = songs[y].grade
          songs[y].grade = gradeSong
          i = -1
          break
        }
        y++
